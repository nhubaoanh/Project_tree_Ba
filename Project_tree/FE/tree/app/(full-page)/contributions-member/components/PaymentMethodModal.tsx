...
const rules: FormRules = {
  amount: { label: "Số tiền", rules: ["required", "number", "integer", "positive"] },
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-[#fffdf5] w-full max-w-2xl rounded-lg shadow-2xl border border-[#d4af37] overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <form id="paymentForm" onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-6">
          <div className="space-y-2">
            <label className="text-sm font-bold text-[#8b5e3c] uppercase">
              Số tiền đóng quỹ <span className="text-red-500">*</span>
            </label>
            <div className="relative">
              <span className="absolute left-3 top-3 text-[#8b5e3c] font-bold">₫</span>
              <input
                type="number"
                name="amount"
                value={values.amount || ""}
                ...
                className={`w-full p-3 pl-8 bg-white border rounded focus:outline-none text-[#333] font-semibold ${
                  getError("amount") ? "border-red-500 bg-red-50" : "border-[#d4af37]/50 focus:border-[#d4af37]"
                }`}
              />
            </div>
            {getError("amount") && <p className="text-red-500 text-sm">{getError("amount")}</p>}
          </div>
              ...
              {PAYMENT_METHODS.map((method) => (
                ...
                >
                  ...
                  <div className="ml-3 flex-1">
                    ...
                    <p className="text-sm text-[#8b5e3c]">{method.description}</p>
                  </div>
                </label>
              ))}
            </div>
            {getError("paymentMethod") && <p className="text-red-500 text-sm">{getError("paymentMethod")}</p>}
          </div>

          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <p className="text-sm text-blue-900">
              <span className="font-bold">ℹ️ Lưu ý:</span> Khi bạn nhấp "Tiếp tục", bạn sẽ được chuyển đến cổng thanh toán. Vui lòng hoàn thành giao dịch để cập nhật quỹ.
            </p>
          </div>
        </form>
